...
interface ParallelSlot {
  ...
  name: string
  phone: string
  status: "ringing" | "connected" | "no-answer" | "cancelled" | "idle"
}
...
export default function DialerClient({ contacts, sessions: initialSessions, pipelineStages }: Props) {
  ...
  const [parallelQueue, setParallelQueue] = useState<Contact[]>([])
  const [parallelAddingToQueue, setParallelAddingToQueue] = useState(false)
  const [parallelSearchQuery, setParallelSearchQuery] = useState("")
  const [dialingParallel, setDialingParallel] = useState(false)
  ...
  // ─── SSE connection for parallel mode ────────────────────────────────────
  useEffect(() => {
    ...
    es.addEventListener("call-answered", (e) => {
      ...
      setParallelSlots(prev => prev.map(s =>
        s.contactId === data.contactId
          ? { ...s, status: "connected" }
          : { ...s, status: s.status === "ringing" ? "cancelled" : s.status }
      ))
    })
    ...
    es.addEventListener("all-missed", () => {
      setParallelSlots(prev => prev.map(s => ({
        ...s,
        status: s.status === "ringing" ? "no-answer" : s.status,
      })))
    })
    return () => es.close()
  }, [parallelMode])
  ...
  async function initTwilioDevice() {
    try {
      ...
      if (token === "mock-token") {
        console.log("[PARALLEL DIALER] Mock mode — Twilio Device not initialized")
        setDeviceReady(true)
        return
      }
      ...
      console.error("[PARALLEL DIALER] Failed to init Twilio Device:", err)
    }
  }

  // ─── Dial 3 contacts in parallel ─────────────────────────────────────────
  async function dialParallel(contactsBatch: Contact[]) {
    const batch = contactsBatch.slice(0, 3)
    setDialingParallel(true)
    setAiAnalysis(null)
    setParallelNotes("")
    setActiveContact(null)
    setActiveParallelCallId(null)
    ...
    try {
      ...
      console.error("[PARALLEL DIALER] Failed to dial:", err)
    } finally {
      setDialingParallel(false)
    }
  }
  ...
  async function analyzeNotes() {
    ...
    try {
      ...
      console.error("[ANALYZE NOTES] Failed:", err)
    } finally {
      setAnalyzingNotes(false)
    }
  }

  // ─── Single-call mode functions ───────────────────────────────────────────
  ...
  const parallelFilteredContacts = contacts
    .filter(c => !!c.phone)
    .filter(c => !parallelQueue.some(q => q.id === c.id))
    .filter(c => {
      const name = `${c.firstName} ${c.lastName}`.toLowerCase()
      return name.includes(parallelSearchQuery.toLowerCase()) || (c.phone || "").includes(parallelSearchQuery)
    })
        ...
        <div className="flex gap-1 bg-gray-100 rounded-lg p-1 w-fit">
          <button
            ...
            className={cn(
              ...
              !parallelMode ? "bg-white text-gray-900 shadow-sm" : "text-gray-600 hover:text-gray-900"
            )}
          >
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="w-80 flex flex-col bg-white border-r overflow-hidden">
            <div className="px-4 py-3 border-b flex items-center justify-between">
              <span className="text-sm font-semibold text-gray-700">Parallel Queue ({parallelQueue.length})</span>
            </div>
            <div className="flex-1 overflow-y-auto">
              {parallelQueue.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-32 text-center p-4">
                  <Users className="w-8 h-8 text-gray-300 mb-2" />
                  <p className="text-sm text-gray-500">Add contacts to dial in parallel</p>
                </div>
              ) : (
                parallelQueue.map((c, idx) => (
                  <div key={c.id} className="flex items-center gap-3 px-4 py-3 border-b">
                    <div className="w-7 h-7 bg-lofty-100 rounded-full flex items-center justify-center text-xs font-bold text-lofty-700 flex-shrink-0">
                      {idx + 1}
                    </div>
                    ...
                    <button
                      onClick={() => setParallelQueue(prev => prev.filter(q => q.id !== c.id))}
            ...
            <div className="border-t">
              <button
                onClick={() => setParallelAddingToQueue(!parallelAddingToQueue)}
              ...
              >
                ...
                {parallelAddingToQueue ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
              </button>
              {parallelAddingToQueue && (
                <div className="border-t">
                  ...
                  <div className="max-h-48 overflow-y-auto">
                    {parallelFilteredContacts.slice(0, 30).map(c => (
                      <button
                        key={c.id}
                        onClick={() => setParallelQueue(prev => [...prev, c])}
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="text-xs font-medium text-gray-900 truncate">{c.firstName} {c.lastName}</div>
                          <div className="text-xs text-gray-500">{formatPhone(c.phone || "")}</div>
                        </div>
                        <Plus className="w-3.5 h-3.5 text-lofty-500 flex-shrink-0" />
                      </button>
                    ))}
                    {parallelFilteredContacts.length === 0 && (
          ...
          <div className="flex-1 flex flex-col overflow-y-auto p-6 gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={cn(
                  ...
                  deviceReady
                    ...
                    : "bg-lofty-600 text-white hover:bg-lofty-700"
                )}
              >
                {deviceReady ? (
                  <>
              ...
              {!deviceReady && (
                <p className="text-xs text-gray-500">Initialize your browser phone to receive connected calls</p>
              )}
            </div>

            {/* 3 Call Slots */}
            <div className="bg-white rounded-xl border p-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">Call Slots</h3>
              <div className="grid grid-cols-3 gap-3">
                {parallelSlots.length === 0
                  ? [0, 1, 2].map(i => (
                      <div key={i} className="border-2 border-dashed border-gray-200 rounded-lg p-4 flex flex-col items-center justify-center h-32 text-center">
                        <Phone className="w-6 h-6 text-gray-300 mb-2" />
                  ...
                  : parallelSlots.map((slot) => (
                      <div
                        key={slot.contactId}
                        className={cn(
                          "rounded-lg border-2 p-4 flex flex-col items-center justify-center h-32 text-center transition-all",
                          slot.status === "connected" && "border-green-400 bg-green-50",
                          slot.status === "ringing" && "border-amber-400 bg-amber-50 animate-pulse",
                          slot.status === "no-answer" && "border-red-300 bg-red-50",
                          slot.status === "cancelled" && "border-gray-200 bg-gray-50",
                          slot.status === "idle" && "border-gray-200",
                        )}
                      >
                        <div className={cn(
                          "w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold mb-2",
                          slot.status === "connected" ? "bg-green-500 text-white" :
                          slot.status === "ringing" ? "bg-amber-500 text-white" :
                          slot.status === "no-answer" ? "bg-red-400 text-white" :
                          "bg-gray-300 text-gray-600"
                        )}>
                          {slot.name.charAt(0)}
                        </div>
                        <div className="text-xs font-semibold text-gray-900 truncate w-full">{slot.name}</div>
                        <div className="text-xs text-gray-500 mt-0.5 truncate w-full">{formatPhone(slot.phone)}</div>
                        <div className={cn(
                          "text-xs font-medium mt-1.5 px-2 py-0.5 rounded-full",
                          slot.status === "connected" && "bg-green-100 text-green-700",
                          slot.status === "ringing" && "bg-amber-100 text-amber-700",
                          slot.status === "no-answer" && "bg-red-100 text-red-600",
                          slot.status === "cancelled" && "bg-gray-100 text-gray-500",
                        )}>
                          {slot.status === "ringing" && "Ringing..."}
                          {slot.status === "connected" && "Connected"}
                          {slot.status === "no-answer" && "No Answer"}
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      {activeContact.phone && (
                        <span className="text-gray-600 text-sm flex items-center gap-1">
                          <Phone className="w-3.5 h-3.5" />{formatPhone(activeContact.phone)}
                        </span>
                      )}
                      {activeContact.email && (
                        <span className="text-gray-500 text-sm">{activeContact.email}</span>
                      )}
                      ...
                      {activeContact.source && (
                        <span className="text-xs text-gray-400">Source: {activeContact.source}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-2 px-3 py-1.5 bg-green-100 rounded-full">
                    <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />
                    <span className="text-green-700 text-sm font-medium">Live Call</span>
                  </div>
                </div>
              </div>
            )}
            ...
            {/* Notes + AI Analysis */}
            {(activeContact || activeParallelCallId) && (
              <div className="bg-white rounded-xl border p-4 space-y-3">
                <label className="text-sm font-semibold text-gray-700 block">Call Notes</label>
                <textarea
                  value={parallelNotes}
                  onChange={e => setParallelNotes(e.target.value)}
                  placeholder="Add notes about this call — property needs, budget, follow-up dates..."
                  rows={4}
                ...
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm font-medium disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  {analyzingNotes ? (
                    <>
                ...
                {aiAnalysis && (
                  <div className="border rounded-lg p-4 bg-purple-50 space-y-3">
                    ...
                    {aiAnalysis.tasks && aiAnalysis.tasks.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-gray-600 mb-1.5">Tasks Created:</p>
                        <ul className="space-y-1">
                          {aiAnalysis.tasks.map((task: any, i: number) => (
                            <li key={task.id || i} className="flex items-start gap-2 text-sm">
                    ...
                    {aiAnalysis.mlsSearch ? (
                      <div>
                        <p className="text-xs font-semibold text-gray-600 mb-1.5">MLS Search Criteria:</p>
                        <div className="text-sm text-gray-700 bg-white rounded p-2 border space-y-0.5">
                          {aiAnalysis.mlsSearch.city && <div>City: <strong>{aiAnalysis.mlsSearch.city}</strong></div>}
                          {aiAnalysis.mlsSearch.minBeds && <div>Min Beds: <strong>{aiAnalysis.mlsSearch.minBeds}</strong></div>}
                          {aiAnalysis.mlsSearch.minBaths && <div>Min Baths: <strong>{aiAnalysis.mlsSearch.minBaths}</strong></div>}
                          {aiAnalysis.mlsSearch.minPrice && <div>Min Price: <strong>${aiAnalysis.mlsSearch.minPrice?.toLocaleString()}</strong></div>}
                          {aiAnalysis.mlsSearch.maxPrice && <div>Max Price: <strong>${aiAnalysis.mlsSearch.maxPrice?.toLocaleString()}</strong></div>}
                          {aiAnalysis.mlsSearch.propertyType && <div>Type: <strong>{aiAnalysis.mlsSearch.propertyType}</strong></div>}
                          {aiAnalysis.mlsSearch.newConstruction && <div className="text-blue-600 font-medium">New Construction</div>}
                        </div>
                        {aiAnalysis.message && (
                          <p className="text-xs text-amber-700 mt-1.5 italic">{aiAnalysis.message}</p>
                        )}
                      </div>
                    ) : (
                      <p className="text-xs text-gray-500 italic">No MLS search criteria detected</p>
                    )}
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() => {
                  if (parallelQueue.length === 0) return
                  const batch = parallelQueue.slice(0, 3)
                  setParallelQueue(prev => prev.slice(3))
                  dialParallel(batch)
                }}
                disabled={parallelQueue.length === 0 || dialingParallel}
                className="flex items-center gap-2 px-6 py-3 bg-lofty-600 text-white rounded-xl hover:bg-lofty-700 font-semibold text-base shadow-sm transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
                {dialingParallel ? (
                  <>
              ...
              {parallelQueue.length > 0 && (
                <span className="text-sm text-gray-500">{parallelQueue.length} contacts in queue</span>
              )}
            </div>
          </div>
