...
function MessageBubble({ msg }: { msg: Message }) {
  ...
  return (
    <div className={`flex ${isUser ? "justify-end" : "justify-start"} mb-3`}>
      {!isUser && <AriaAvatar size={28} className="rounded-full mr-2 flex-shrink-0 mt-0.5" />}
      <div
...
function TypingIndicator({ thinking }: { thinking: boolean }) {
  return (
    <div className="flex justify-start mb-3">
      <AriaAvatar size={28} className="rounded-full mr-2 flex-shrink-0" />
...
export default function AIAssistant() {
  ...
  // Greet Catherine automatically when she opens the CRM (once per session).
  useEffect(() => {
    let seen = false
    try { seen = !!sessionStorage.getItem("aria_seen") } catch { /* noop */ }
    if (seen) return
    const t = setTimeout(() => {
      setOpen(true)
      try { sessionStorage.setItem("aria_seen", "1") } catch { /* noop */ }
    }, 1400)
  ...
  return (
    <>
        ...
        >
          ...
          <div className="bg-[#1a3a5c] px-4 py-3 flex items-center justify-between flex-shrink-0">
            <div className="flex items-center gap-2.5">
              <AriaAvatar size={36} className="rounded-full ring-2 ring-[#c9a84c]/60" />
              <div>
          ...
          <div className="flex-1 overflow-y-auto p-4 min-h-0">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center px-2">
                <AriaAvatar size={60} className="rounded-full mb-3 ring-2 ring-[#c9a84c]/40" />
      ...
      <button
        ...
        className="fixed bottom-4 right-4 z-50 rounded-full shadow-lg flex items-center justify-center transition-all hover:scale-105 active:scale-95 overflow-hidden"
        style={{ width: 54, height: 54, background: open ? "#c9a84c" : "#1a3a5c" }}
      >
        {open ? (
          <X className="w-5 h-5 text-white" />
        ) : (
          <>
            <AriaAvatar size={54} className="rounded-full" />
